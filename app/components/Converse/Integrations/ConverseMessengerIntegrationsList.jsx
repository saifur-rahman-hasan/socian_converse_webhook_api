...
import {useDeleteWorkspaceChannelMutation} from "@/store/features/workspace/WorkspaceAPISlice";


export default function ConverseMessengerIntegrationsList({ channels }) {
	const router = useRouter()
	const { workspaceId } = router?.query

	const messengerChannels = collect(channels).where('integrationId', 'messenger').toArray()
		...
		<div className="">
			<div className="sm:flex sm:items-center">
				<div className="sm:flex-auto">
					<h1 className="text-base font-semibold leading-6 text-gray-900">Messenger Channels</h1>
								...
								{messengerChannels.map((channel) => (
									<tr key={`telegram_channel_${channel?._id}`}>
										<td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
											{ channel?.sourceConfig?.name || channel?.sourceId }
										</td>
										...
										<td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 flex items-center justify-end gap-x-4">
											<Link
